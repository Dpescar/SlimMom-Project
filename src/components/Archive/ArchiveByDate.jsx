  ...
  alldates.forEach(function (alldate, index) {
    menuItems.push(
      <MenuItem key={index} value={index}>
        Archive Date {alldate._id.archiveDate}
        From {alldate._id.startDate} To {alldate._id.endDate}{' '}
      </MenuItem>
    );
  });
    ...
    <>
      <div className={css.report}>
        ...
        <div ref={printRef} id="element-to-download-as-pdf">
          ...
          <h5 className={css.centerText}>
            {userinfo[0].name} ({userinfo[0].email})
          </h5>
          <h5 className={css.centerText}>
            Archived on {diaryinf[0].archiveDate}
          </h5>
          <h5 className={css.centerText}>
            {diaryinf[0].startDate} through {diaryinf[0].endDate}
          </h5>

          {calcinfo[0].calculatorEntries[0].calculatorEntry[0].unitOfMeasure !==
          'S' ? (
            <>
              <h5 className={css.centerText}>
                Height:{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].height.toString()}
                cm Age:{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].age.toString()}{' '}
                BloodType:{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].bloodType}{' '}
              </h5>
              <h5 className={css.centerText}>
                Current Weight:{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].currentWeight.toString()}
                kg Desired Weight:{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].desiredWeight.toString()}
                kg
              </h5>
            </>
          ) : (
            <>
              <h5 className={css.centerText}>
                Height:{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].heightFeet.toString()}{' '}
                feet{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].heightInch.toString()}{' '}
                inches Age:{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].age.toString()}{' '}
                BloodType:{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].bloodType}{' '}
              ...
              <h5 className={css.centerText}>
                Current Weight:{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].currentWeightLbs.toString()}
                lbs Desired Weight:{' '}
                {calcinfo[0].calculatorEntries[0].calculatorEntry[0].desiredWeightLbs.toString()}
                lbs
              </h5>
            </>
          )}
          ...
          {diaryinf.map((item, index) => (
            <div key={`archiveDate-${index}`}>
              ...
              <div className={css.row}>
                <h4 key={`date-${index}`} className={css.dateHeader}>
                  {' '}
                  {item.date}
                </h4>
              </div>
              <div key={`rowTitle-${index}`} className={css.row}>
                <h4 key={`FoodTitle-${index}`} className={css.firstColumn}>
                  Food Name
                </h4>
                <h4 key={`gramsTitle-${index}`} className={css.otherColumn}>
                  {' '}
                  Grams
                </h4>
                <h4 key={`calsTitle-${index}`} className={css.otherColumn}>
                  Calories
                </h4>
              </div>
              ...
              {item.foodItems.map((food, rowID) => (
                <>
                  <div className={css.row} key={`foodGeroup-${rowID}`}>
                    <p key={`title-${rowID}`} className={css.firstColumn}>
                      {food.title}
                    </p>
                    <p key={`weight-${rowID}`} className={css.otherColumn}>
                      {food.weight}
                    </p>
                    <p key={`calories-${rowID}`} className={css.otherColumn}>
                      {food.calories}
                    </p>
                  </div>
                  {}
              ...
              <div key={`totalHead-${index}`} className={css.row}>
                ...
                <p
                  key={`totalHeadconsumed-${index}`}
                  className={css.totalColumn}
                >
                  Calories consumed
                </p>
                <p
                  key={`totalHeadRemaining-${index}`}
                  className={css.totalColumn}
                >
                  Remaining calories
                </p>
              ...
              <div key={`totalamtd-${index}`} className={css.row}>
                ...
                <p
                  key={`totalamtconsumed-${index}`}
                  className={css.totalColumn}
                >
                  {(
                    Math.round(
                      item.foodItems.reduce(function (prev, cur) {
